...
@section('title',SiteHelpers::ayar('author').' | Yeni Kategori ')
@section('page-css')
    <!-- Dropify css -->
            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box d-flex align-items-center justify-content-between">
                        <h4 class="mb-0 font-size-18">Yeni Kategori</h4>
                            ...
                            <div class="col-sm-8">
                                <form class="form form-vertical" method="post"
                                    ...
                                    <div class="card-body col-sm-6 mb-5">
                                        <h4 class="card-title">Header Resmi</h4>
                                        <p class="card-subtitle mb-4" style="color: red;font-weight: bold">Yanlızca 1080X1080 px ölçülerinde</p>
                                        <input type="file"
                                               class="dropify" name="cat_bg"
                                               data-max-file-size="2M"
                                               data-default-file="{{ asset($category->cat_bg ?? '') }}"/>
                                    </div> <!-- end card-body-->

                                    <div class="col-12">
                                        <div class="form-group">
                                            <label for="ust_id">Kategorisini Seçiniz</label>
                                            <select name="ust_id" class="form-control" data-toggle="select2">
                                                <option><-- Kategori Seçiniz --></option>
                                                <option value="null" {{ (isset($category) && $category->ust_id === null) ? 'selected' : '' }}>Ana Kategori</option>
                                                @foreach ($categories as $cat)
                                                    <option value="{{ $cat->id }}" {{ (isset($category) && $category->ust_id == $cat->id) ? 'selected' : '' }}>
                                                        {{ $cat->cat_name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="col-3">
                                        <div class="form-group">
                                            ...
                                            <input type="text" data-toggle="touchspin" name="must"
                                                   value="{{ old('must', $category->must ?? $maxMust + 1) }}" required>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-12">
                                        <div class="form-check form-switch">
                                            ...
                                            <input type="checkbox" data-toggle="switchery" data-color="#38b3d6" name="isActive" value="1"
                                                {{ (isset($category) && $category->isActive) || old('isActive', true) ? 'checked' : '' }}/>
                                    ...
                                    <div class="d-flex mt-3" style="gap: 20px;">
                                        ...
                                        <div class="tab-content flex-fill" id="v-pills-tabContent-right">
                                            ...
                                            @include('backend.category.en-tab')
                                            <!-- Almanca Dil Dosyaları -->
                                            @include('backend.category.de-tab')
                                            <!-- İspanyolca Dil Dosyaları -->
                                            @include('backend.category.es-tab')
                                            <!-- Fransızca Dil Dosyaları -->
                                            @include('backend.category.fr-tab')
                                            <!-- Macarca Dil Dosyaları -->
                                            @include('backend.category.hu-tab')
                                            <!-- İtalyanca Dil Dosyaları -->
                                            @include('backend.category.it-tab')
                                            <!-- Sırpça Dil Dosyaları -->
                                            @include('backend.category.sr-tab')
                                        </div>
                                    ...
                                    <div class="card-footer">
                                        <button class="btn btn-warning waves-effect waves-light">Ekle</button>
                                    </div>
                                </form>
                            </div> <!-- end col-->
